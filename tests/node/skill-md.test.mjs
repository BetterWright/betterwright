...
// with `node bin/betterwright.mjs skill --claude > SKILL.md` after editing
// SKILL_PREAMBLE or the operator guidance in src/prompt.mjs.
test("SKILL.md matches `betterwright skill --claude` output", () => {
  const root = fileURLToPath(new URL("../..", import.meta.url));
  const generated = execFileSync(
    process.execPath,
    ["bin/betterwright.mjs", "skill", "--claude"],
  ...
  assert.equal(
    checkedIn,
    generated,
    "SKILL.md is stale — run: node bin/betterwright.mjs skill --claude > SKILL.md",
  );
});
