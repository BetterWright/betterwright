...
- **Do not click reCAPTCHA Skip after tile picks.** `#recaptcha-verify-button`
  ...
  Verify: a known Verify/Next name, a selected image-grid tile, or a label
  that changed after the last tile pick. Known Skip names are left alone, and
  an unrecognized locale word is not treated as Verify on its own. Agents are
  told that replacement photo grids are the same image-grid stage, so they
  keep picking instead of handing off after three photo swaps.
