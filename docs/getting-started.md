...
### Idle CPU, memory, and page parking

A headless Chromium page is never hidden on its own. Playwright also enables
focus emulation on every page, which keeps each tab "visible" to Chromium. So
none of the machinery a headed browser uses to stop paying for background tabs
ever engages: the frame loop free-runs at the host refresh rate, and any page
with a spinner, a carousel, or a canvas keeps a core busy for as long as the
session lives. Nothing in the launch arguments changes this.

So BetterWright parks a session's pages once its last execution unwinds. It
pauses animation timelines, briefly releases focus emulation, and freezes the
page through Chromium's native lifecycle, exactly as a tab the user switched
away from. A page whose heap grew since it was last parked also gets one full
garbage collection, so the memory its previous work freed is returned to the
operating system. Before the next execution starts, the page is resumed,
focused, and visible again, with its timers, animation frames, and in-page
state intact. The parked window is the model's thinking time, which is where
most of the waste was. See [runtime performance](runtime-performance.md) for
measurements.

This is on by default and needs no configuration. It never applies in headed
mode or while a live view is streaming, and it only freezes browsers
