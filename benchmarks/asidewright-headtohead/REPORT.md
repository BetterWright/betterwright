# BetterWright vs AsideWright — head-to-head

A physical, runtime-vs-runtime comparison on this machine. Both `betterwright`
and the `aside` CLI accept raw browser-automation JavaScript, so this isolates
...
`aside` CLI against its always-on daemon browser. Recorded 2026-07-18.

## Results

### 1. Latency — navigate + interactive snapshot

| Mode | BetterWright | AsideWright |
| --- | --- | --- |
| Cold one-shot CLI call (median of 3) | ~700 ms | ~535 ms |
| Warm per-op (same session, after first) | **68–109 ms** | ~90–130 ms* |

AsideWright's one-shot CLI call is faster because its browser is already running
in a persistent daemon — each `aside repl` only pays CLI startup + a daemon
...
package; AsideWright via its daemon). There BetterWright's per-op is **68–109 ms**
— on par with or slightly faster than AsideWright. First op in a BetterWright
session is ~550 ms (one-time browser launch), amortized across the session.

\* AsideWright's pure browser-op time is inside its ~520 ms CLI wall-clock and
not separately reported; the daemon op itself is comparable to BetterWright's
warm op.

### 2. Capability parity

| Task | BetterWright | AsideWright |
| --- | --- | --- |
...
(`textbox "Password" … : hunter2secret`), where AsideWright emits `[redacted]`.
...
wall-clock: `aside exec` (Aside's agent + AsideWright) vs the **Pi coding agent +
...
on both sides (`aside --effort low`, `pi --thinking low`), so only the
agent-scaffold + runtime differ.

| Task (login-free, verifiable) | AsideWright | Pi + BetterWright | Both correct? |
| --- | --- | --- | --- |
...
reasoning effort matched, AsideWright is still ~2–3× faster end-to-end. Since the
browser runtime itself is at parity (section 1: ~70–110 ms/op both), the gap is
**not the runtime** — it is (a) Aside's purpose-built, browser-tuned agent loop
taking fewer/tighter steps than Pi's general coding-agent loop, and (b) Aside's
...
- **Capability:** at parity on the core read/act/multi-tab loop and password
  redaction.
- **Architecture difference (by design):** AsideWright drives the user's
...
- **End-to-end:** both agents complete the same tasks correctly. AsideWright's
...
autofill) vs AsideWright's, and a passkey sign-in once CDP virtual-authenticator
support lands.
