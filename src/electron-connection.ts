...
import { BetterwrightCdpTarget } from "./electron-cdp.js";
...
export async function openBetterwrightConnection(
  ...
  cookieImport = false,
  expectAgentInput?: ElectronHostOptions["expectAgentInput"],
) {
  if (!contents.debugger.isAttached()) contents.debugger.attach("1.3");
  // SAFETY: Target.getTargetInfo is a trusted Electron CDP reply; validate its ID below.
  const { targetInfo } = (await contents.debugger.sendCommand("Target.getTargetInfo")) as {
    targetInfo: { targetId: string };
  };
  if (!isString(targetInfo?.targetId) || !targetInfo.targetId) throw new Error("Browser target unavailable.");
  // SAFETY: Target.attachToTarget is a trusted Electron CDP reply; validate the session below.
  const { sessionId: backendSessionId } = (await contents.debugger.sendCommand(
    "Target.attachToTarget",
    { targetId: targetInfo.targetId, flatten: true },
  )) as { sessionId: string };
  if (!isString(backendSessionId) || !backendSessionId) throw new Error("Browser session unavailable.");
  ...
  server.on("upgrade", (request, socket, head) => {
    ...
    sockets.handleUpgrade(request, socket, head, (client) => {
      ...
      client.on("message", (data) => {
        try {
          const message: UntrustedValue = JSON.parse(data.toString());
          if (!message || !isRecord(message) || Array.isArray(message)) throw new Error();
