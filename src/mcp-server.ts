...
On challenges keep the page; captcha.solve() first (local checkbox/turnstile/slider/motion/drag-fit); if 'processing', open the numbered crop, then captcha.solve({tiles:[indexes]}). Fallbacks: captcha.detect, captcha.inspect, captcha.click, captcha.drag, captcha.readText, captcha.clickTiles, human.click. Max three distinct stages; a rejected action = stop, use an alternate source or human handoff. After clearing verify state; replay only if idempotent or provably incomplete. Never duplicate a submission, purchase, or message.`;
