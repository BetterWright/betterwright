...
// live-view-html.mjs. Kept functional so tests and bare embedders still work.
...
export function createLiveViewServer({
  ...
  function postChat({ role = "agent", text = "", kind } = {}) {
  ...
  function servePage(response, markup, extraHeaders = {}) {
  ...
  function beginHandoff(prompt, { timeoutMs } = {}) {
    if (!running) {
  ...
  function beginAsk(question, { options: choiceList = [], timeoutMs } = {}) {
    if (!running) {
      return Promise.reject(new Error("Live view is not running."));
    }
    if (ask) {
