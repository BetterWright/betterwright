...
- Bulk repetitive work: prefer the site's own machinery to repeating UI actions — find the underlying endpoint or in-page function (its network calls, its app bundle) and drive it with `page.evaluate`, then confirm the result in the UI like any other action. Never to bypass a paywall, rate limit, or access control.
